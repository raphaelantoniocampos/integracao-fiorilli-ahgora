...
from app.domain.entities import AutomationTask, AutomationTaskType, AutomationTaskStatus
...
async def test_generate_tasks_dfs_dismissed(sync_service):
    job_id = uuid4()
    fiorilli_df = pd.DataFrame([
        {"id": "000001", "name": "USER", "dismissal_date": "01/01/2024", "binding": "CLT"}
    ])
    ahgora_df = pd.DataFrame([
        {"id": "000001", "name": "USER", "dismissal_date": None}
    ])
